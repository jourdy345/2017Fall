...
\begin{problemAnswer}{
  Let $H$ be the event of obtaining a head and $S_{i}$ the event of selecting the $i$th coin. Then
  \begin{equation}
    P(S_{i} \mid H) = \dfrac{P(H\mid S_{i})P(S_{i})}{\sum_{i=1}^{5}P(H\mid S_{i})P(S_{i})} = \dfrac{2}{5}p_{i}\quad \left(\because p_{i}=P(H\mid S_{i})\text{ and } P(S_{i})=\dfrac{1}{5}\right)
  \end{equation}
} \end{problemAnswer}
...
\begin{problemAnswer}{
  \begin{equation}
    P(H\mid S_{i})P(S_{i}\mid H) = p_{i}\cdot \dfrac{2}{5}p_{i} = \dfrac{2}{5}p_{i}^{2}\quad \left(\because P(H\mid S_{i},H)= P(H\mid S_{i}) \text{ by independent trials}\right)
  \end{equation}
}\end{problemAnswer}
...
\begin{problemAnswer}{
  \begin{equation}
    P(H\mid S_{i})P(S_{i}\mid H^{c}) = p_{i}\cdot \dfrac{P(H^{c}\mid S_{i})P(S_{i})}{\sum_{i=1}^{6}P(H^{c}\mid S_{i})P(S_{i})} = \dfrac{2}{5}p_{i}(1-p_{i})
  \end{equation}
  \begin{remark}
    (b) and (c) are related to \textbf{the posterior predictive probability} where we incorporate the previously observed information.
  \end{remark}
}\end{problemAnswer}
...
\begin{problemAnswer}{
  \begin{align}
  M_{X}(t) &= \dfrac{1}{2}\int_{-\infty}^{\infty}e^{-|x|+tx}\,dx\\
  &= \dfrac{1}{2}\left(\int_{-\infty}^{0}e^{(1+t)x}\,dx+\int_{0}^{\infty}e^{(-1+t)x}\,dx \right)\\
  &= \dfrac{1}{2}\left(\left. \dfrac{1}{1+t}e^{(1+t)x}\right|_{-\infty}^{0}+\left.\dfrac{1}{-1+t}e^{(-1+t)x}\right|_{0}^{\infty} \right)\\
  &= \dfrac{1}{2}\left(\dfrac{1}{1+t}-\dfrac{1}{-1+t} \right)\\
  &= \dfrac{1}{1-t^{2}}
  \end{align}
  Then
  \begin{equation}
  K_{X}(t) = -\log\left(1-t^{2}\right)
  \end{equation}
  Thus,
  \begin{equation}
  \begin{split}
    \kappa_{1}&=\left.\dfrac{d}{dt}K_{X}(t)\right|_{t=0} = 0\\
    \kappa_{2} &= \left.\dfrac{2(t^{2}+1)}{\left(1-t^{2}\right)^{2}}\right|_{t=0}=2\\
    \kappa_{3} &= \left.-\dfrac{4t(t^{2}+3)}{(t^{2}-1)^{3}}\right|_{t=0}=0
  \end{split}
  \end{equation}
}\end{problemAnswer}
\end{homeworkProblem}
